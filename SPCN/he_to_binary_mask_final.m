...
function [binary_mask, shape_marker, color_mask]=he_to_binary_mask(filename, ext)
im_file=strcat(filename, ext);
...
for zz=1:length(xy) %for each region
    ...
    % find area
    s = regionprops(polygon,'area');
    areas = cat(1,s.Area);
   
    % erode according to cell area
    if (max(max(polygon)) ~= 0)
        newSE = diskValue(areas(1));
        se = strel('disk',ceil(newSE));
        cellShapeMarker = imerode(polygon,se);
        shape_marker = shape_marker + cellShapeMarker;
    end
